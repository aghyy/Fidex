...
export default function AccountsManager() {
  ...
  // creation handled by CreateAccountDialog in page header

  async function handleDelete(id: string) {
    setState("loading");
    setError(null);
    try {
      const res = await fetch(`/api/account/${id}`, { method: "DELETE", credentials: "include" });
  ...
  async function handleUpdate(id: string, updates: Partial<Pick<Account, "name" | "accountNumber" | "color" | "icon" | "balance">>) {
    setState("loading");
    setError(null);
    try {
  ...
  return (
    <div className="space-y-6">
      {/* Create button moved to page header via CreateAccountDialog */}
                ...
                <MorphingDialogContainer>
                  <MorphingDialogContent className="w-full max-w-lg rounded-2xl border bg-background p-5 shadow-xl">
                    ...
                    <form
                      onSubmit={(e) => {
                        e.preventDefault();
                        const item = accounts.find((x) => x.id === c.id);
                        if (!item) return;
                    ...
                    >
                      <div>
                        ...
                        <input
                          ...
                          value={c.name}
                          onChange={(e) => setAccounts((prev) => prev.map((x) => (x.id === c.id ? { ...x, name: e.target.value } : x)))}
                        />
                      </div>
                      <div>
                        ...
                        <div className="mt-2 grid grid-cols-8 gap-2">
                          {ICON_OPTIONS.map((icon) => (
                            <button
                              key={icon}
                              type="button"
                              onClick={() => setAccounts((prev) => prev.map((x) => (x.id === c.id ? { ...x, icon } : x)))}
                      ...
                      <div>
                        ...
                        <div className="mt-2 flex items-center gap-2 flex-wrap">
                          <input
                            ...
                            onChange={(e) => setAccounts((prev) => prev.map((x) => (x.id === c.id ? { ...x, color: e.target.value } : x)))}
                          />
                          {DEFAULT_COLORS.map((col) => (
                            <button
                              key={col}
                              type="button"
                              onClick={() => setAccounts((prev) => prev.map((x) => (x.id === c.id ? { ...x, color: col } : x)))}
